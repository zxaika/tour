...
async function loadBookings() {
    try {
        const response = await fetch('/api/admin/bookings', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ password: adminPassword, filter: currentFilter })
        });
...
function downloadReceipt(bookingId) {
    window.open(`/api/admin/receipt/${bookingId}.pdf?password=${encodeURIComponent(adminPassword)}`, '_blank');
}
...
async function dropBookingToDate(event, newDate) {
    ...
    try {
        const response = await fetch('/api/admin/move-booking', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ password: adminPassword, bookingId, newCheckIn: newDate })
        });
