...
const TG_STATE_FILE = path.join(__dirname, 'data', 'telegram-state.json');
...
function dateRange(startStr, endStr, includeEnd = false) {
    const dates = [];
    const start = new Date(startStr);
    const end = new Date(endStr || startStr);
    for (let d = new Date(start); includeEnd ? d <= end : d < end; d.setDate(d.getDate() + 1)) {
        dates.push(d.toISOString().split('T')[0]);
    }
    return dates;
}

function isClosedDate(dateStr, prices = loadPrices()) {
    return Boolean(prices.closedDates && prices.closedDates[dateStr]);
}

function hasClosedDates(checkIn, checkOut, type, prices = loadPrices()) {
    const dates = type === 'tour' ? dateRange(checkIn, checkOut || checkIn, true) : dateRange(checkIn, checkOut, false);
    return dates.find(d => isClosedDate(d, prices));
}

function getRoomNightPrice(roomId, dateStr, prices, excludeBookingId = null) {
    const roomKey = String(roomId);
    const base = Number(prices.roomDatePrices?.[roomKey]?.[dateStr] || prices.rooms[roomKey]?.price || 0);
    return applyDynamicPrice(base, dateStr, prices, excludeBookingId);
}

function getRoomOccupancyPercent(dateStr, prices, excludeBookingId = null) {
    const roomIds = Object.keys(prices.rooms || {});
    if (!roomIds.length) return 0;
    const occupied = new Set();
    getActiveBookings().forEach(booking => {
        if ((booking.type || 'room') === 'tour') return;
        if (excludeBookingId && String(booking.id) === String(excludeBookingId)) return;
        eachBookedDay(booking, (day) => {
            if (day === dateStr && booking.roomId) occupied.add(String(booking.roomId));
        });
    });
    return Math.round((occupied.size / roomIds.length) * 100);
}

function getOccupancyPercentIncrease(dateStr, prices, excludeBookingId = null) {
    const rules = prices.occupancyRules || {};
    if (!rules.enabled) return 0;
    const occupancy = getRoomOccupancyPercent(dateStr, prices, excludeBookingId);
    const matched = [...(rules.thresholds || [])]
        .filter(rule => occupancy >= Number(rule.occupancy))
        .sort((a, b) => Number(b.occupancy) - Number(a.occupancy))[0];
    return matched ? Number(matched.percent || 0) : 0;
}

function applyDynamicPrice(basePrice, dateStr, prices, excludeBookingId = null) {
    const rules = prices.occupancyRules || DEFAULT_PRICES.occupancyRules;
    if (rules.enabled === false) return Number(basePrice || 0);
    const percent = getOccupancyPercentIncrease(dateStr, prices, excludeBookingId);
    return Math.round(Number(basePrice || 0) * (1 + percent / 100));
}


function getTourDatePrice(tourId, tourName, dateStr, prices) {
    const entry = Object.entries(prices.tours).find(([id, t]) => String(id) === String(tourId) || t.name === tourName);
    if (!entry) return 0;
    const [id, tour] = entry;
    return Number(prices.tourDatePrices?.[String(id)]?.[dateStr] || tour.price || 0);
}

function getToursInfo() {
    const prices = loadPrices();
    return Object.entries(prices.tours).map(([id, t]) => ({ id: Number(id), ...t }));
}

const TOURS_INFO = [
    { id: 1, name: 'Сванетия', price: 300, duration: '2 дня' },
...
function eachBookedDay(booking, cb) {
    const start = new Date(booking.checkIn);
    const end = new Date(booking.checkOut || booking.checkIn);
    if (booking.type === 'tour') {
        const last = booking.checkOut && booking.checkOut !== booking.checkIn ? end : start;
...
function isRoomAvailable(roomId, checkIn, checkOut) {
    const prices = loadPrices();
    if (hasClosedDates(checkIn, checkOut, 'room', prices)) return false;
    const data = loadBookings();
    const checkInDate = new Date(checkIn);
    const checkOutDate = new Date(checkOut);
    return !data.bookings.some(booking => {
        if (booking.status === 'cancelled' || booking.type === 'tour' || String(booking.roomId) !== String(roomId)) return false;
        const bookingIn = new Date(booking.checkIn);
        const bookingOut = new Date(booking.checkOut);
        return !(checkOutDate <= bookingIn || checkInDate >= bookingOut);
    });
}

function listMediaFiles() {
    const result = [];
    const imageDir = path.join(PUBLIC_DIR, 'images', 'mainimg');
    const videoDir = path.join(PUBLIC_DIR, 'videos', 'gallery');
    ensureDir(imageDir);
    ensureDir(videoDir);
    fs.readdirSync(imageDir).filter(name => /\.(jpg|jpeg|png|webp)$/i.test(name)).forEach(name => {
        result.push({ type: 'image', src: `images/mainimg/${name}`, title: name });
    });
    fs.readdirSync(videoDir).filter(name => /\.(mp4|webm|mov)$/i.test(name)).forEach(name => {
        result.push({ type: 'video', src: `videos/gallery/${name}`, title: name });
    });
    return result.sort((a, b) => b.src.localeCompare(a.src));
}


function listRoomImageFiles() {
    const result = {};
    const imageDir = path.join(PUBLIC_DIR, 'images', 'rooms');
    ensureDir(imageDir);
    fs.readdirSync(imageDir)
        .filter(name => /\.(jpg|jpeg|png|webp)$/i.test(name))
        .forEach(name => {
            const match = name.match(/^room-(\d+)-/i);
            if (!match) return;
            const id = match[1];
            if (!result[id]) result[id] = [];
            result[id].push(`images/rooms/${name}`);
        });
    Object.values(result).forEach(list => list.sort((a, b) => b.localeCompare(a)));
    return result;
}

function listTourImageFiles() {
    const result = {};
    const imageDir = path.join(PUBLIC_DIR, 'images', 'tours');
    ensureDir(imageDir);
    fs.readdirSync(imageDir)
        .filter(name => /\.(jpg|jpeg|png|webp)$/i.test(name))
        .forEach(name => {
            const match = name.match(/^tour-(\d+)-/i);
            if (!match) return;
            const id = match[1];
            if (!result[id]) result[id] = [];
            result[id].push(`images/tours/${name}`);
        });
    Object.values(result).forEach(list => list.sort((a, b) => b.localeCompare(a)));
    return result;
}
...
function loadTgState() {
    initData();
    if (!fs.existsSync(TG_STATE_FILE)) return {};
    try { return JSON.parse(fs.readFileSync(TG_STATE_FILE, 'utf8')); } catch { return {}; }
}

function saveTgState(state) {
    ensureDir(path.dirname(TG_STATE_FILE));
    fs.writeFileSync(TG_STATE_FILE, JSON.stringify(state, null, 2));
}
...
app.get('/api/available-rooms', (req, res) => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(checkIn || '')) || !/^\d{4}-\d{2}-\d{2}$/.test(String(checkOut || ''))) {
        return res.status(400).json({ error: 'Укажите даты checkIn и checkOut в формате YYYY-MM-DD' });
    }
    if (new Date(checkOut) <= new Date(checkIn)) {
        return res.status(400).json({ error: 'Дата выезда должна быть позже даты заезда' });
    }
    const prices = loadPrices();
    const nights = dateRange(checkIn, checkOut, false).length;
    const rooms = Object.entries(prices.rooms).filter(([id]) => isRoomAvailable(id, checkIn, checkOut)).map(([id, room]) => ({
        id: Number(id),
        name: room.name,
        price: room.price,
        total: calculateBookingPrice({ type: 'room', roomId: id, checkIn, checkOut, prices })
    }));
    res.json({ checkIn, checkOut, nights, rooms });
});
...
app.post('/api/quote', (req, res) => {
    ...
    const prices = loadPrices();
    const closedDate = hasClosedDates(checkIn, checkOut, type || 'room', prices);
    if (closedDate) return res.status(400).json({ error: `Бронирование закрыто на дату ${formatRuDate(closedDate)}` });
    const total = calculateBookingPrice({ type, roomId, tourId, tourName, checkIn, checkOut, prices });
    res.json({ total });
});
...
app.post('/api/check-availability', (req, res) => {
    ...
    const closedDate = hasClosedDates(checkIn, checkOut, 'room', prices);
...
📅 ${booking.type === 'tour' ? 'Дата тура' : 'Заезд'}: ${formatDate(booking.checkIn)}${booking.type === 'tour' && booking.checkOut === booking.checkIn ? '' : `
📅 ${booking.type === 'tour' ? 'Окончание' : 'Выезд'}: ${formatDate(booking.checkOut)}`}
...
function calculateBookingPrice({ type, roomId, tourId, tourName, checkIn, checkOut, clientTotal, prices }) {
    if ((type || 'room') === 'tour') {
        return getTourDatePrice(tourId, tourName, checkIn, prices) || Number(clientTotal || 0);
    }
    const nights = dateRange(checkIn, checkOut, false);
...
app.post('/api/bookings', async (req, res) => {
    ...
    const newBooking = {
        ...
        checkIn,
        checkOut,
        totalPrice: calculateBookingPrice({ type, roomId, tourId, tourName, checkIn, checkOut, guestsCount, clientTotal: totalPrice, prices }),
        status: 'confirmed',
        notes: notes || '',
...
app.post('/api/admin/move-booking', (req, res) => {
    const { password, bookingId, newCheckIn } = req.body;
    if (password !== process.env.ADMIN_PASSWORD) return res.status(401).json({ error: 'Неверный пароль' });
    if (!/^\d{4}-\d{2}-\d{2}$/.test(newCheckIn || '')) return res.status(400).json({ error: 'Неверная дата' });
    const data = loadBookings();
    const prices = loadPrices();
    ...
    const oldIn = new Date(booking.checkIn);
    const oldOut = new Date(booking.checkOut || booking.checkIn);
    ...
    const closedDate = hasClosedDates(newCheckIn, newOutStr, booking.type || 'room', prices);
    ...
    if ((booking.type || 'room') !== 'tour') {
        for (const other of data.bookings) {
            ...
            if (!(newOut <= new Date(other.checkIn) || newIn >= new Date(other.checkOut))) {
...
function getPdfFontPath() {
    // PDFKit стандартными шрифтами не показывает кириллицу.
    // Поэтому ищем системный TTF/OTF-шрифт с Unicode: на VPS обычно есть DejaVu или Noto.
    // При необходимости можно указать путь вручную в .env: PDF_FONT_PATH=/path/to/DejaVuSans.ttf
    const candidates = [
...
app.get('/api/admin/receipt/:id.pdf', (req, res) => {
    if (req.query.password !== process.env.ADMIN_PASSWORD) return res.status(401).send('Неверный пароль');
    const data = loadBookings();
...
// ========== TELEGRAM BOT: просмотр туров и активных заявок ==========
let tgOffset = 0;
